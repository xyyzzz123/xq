...
const Stream = {
  ...
  state: function(name) {
    return {
      new_queue_size: this._streams[name]['new'].length,
      retry_queue_size: this._streams[name]['retry'].length,
      max_concurrency: this._streams[name]['maxConcurrency'],
      cur_concurrency: this._streams[name]['curConcurrency'],
      cur_interval: this._streams[name]['interval'],
      success_num: this._streams[name]['success'],
      fail_num: this._streams[name]['fail']
    };
  },
